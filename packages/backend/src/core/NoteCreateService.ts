...
export class NoteCreateService implements OnApplicationShutdown {
	...
	public async fetchAndCreate(user: {
		...
		if (data.visibility === 'public' && data.channelId == null) {
                        console.log('-----------------too many public note-----------');
                        const cycle = (await this.roleService.getUserPolicies(user.id)).publicMinimumInterval;
                        console.log(cycle);
                        if ( ( cycle > 0 ) && ( cycle < 10000) ){
                                ...
                                const count = await this.notesRepository.countBy({
                                 ...
                                 console.log(count);
                                 if ( count > 0 ){
				      throw new IdentifiableError('a817376a-4bcc-11ee-8fd9-15e99585d2ed', 'Interval is too short. Please note after few minutes.');
                                 }
                        }
                }
		...
		if (data.visibility === 'specified'){
				     ...
				     throw new IdentifiableError('4653904a-2567-5f3a-2cad-d061dc38e676', 'Too many destinations for this mention.');
                               }
                               const cycle = (await this.roleService.getUserPolicies(user.id)).mentionMinimumInterval;
                               console.log(cycle);
                               if ( ( cycle > 0 ) && ( cycle < 10000) ){
                                       const last_date = new Date(Date.now() - (cycle * 1000 * 60));
                                       const last_id = this.idService.gen( last_date.getTime() );
                                       ...
                                       console.log(count);
                                       if ( count > 0 ){
                                               console.log('--too meny mention error');
                                               exec('echo "quicky mention:'+user.id+'"|mailx -r misskey@jay.misskey.day -s \"mentionError\" sabakan432108794321@misskey.day', (err, stdout, stderr) => {
                                               if (err) { console.log(err); }
                                                       console.log(stdout);
                                               });
                                               console.log('--end of mail');
				      		throw new IdentifiableError('d5f820ab-13d3-7e50-dad9-74225cd190d0', 'Interval is too short. Please mention after few minutes.');
                                       }
                               }
                }
